...
impl Patch {
	...
	pub fn run(self) -> Result<(), String> {
		...
		let path = self
			.path
			.map(|p| if !p.exists() { Err(format!("Given --path=`{}` does not exist!", p.display())) } else { Ok(p) })
		...
		let point_to = PointTo::from_cli(self.point_to_git, self.point_to_git_branch, self.point_to_git_commit)?;
	...
	fn patch_target(&self) -> Result<PatchTarget, String> {
		if let Some(ref custom) = self.target {
			...
			Ok(PatchTarget::Git("https://github.com/paritytech/substrate".into()))
		} else if self.polkadot {
			Ok(PatchTarget::Git("https://github.com/paritytech/polkadot".into()))
		} else if self.cumulus {
			Ok(PatchTarget::Git("https://github.com/paritytech/cumulus".into()))
		} else if self.beefy {
			Ok(PatchTarget::Git("https://github.com/paritytech/parity-bridges-gadget".into()))
		} else if self.crates {
...
fn workspace_root_package(path: &Path) -> Result<PathBuf, String> {
	...
	let metadata = cargo_metadata::MetadataCommand::new()
		.current_dir(path)
		.exec()
		.map_err(|e| format!("Failed to get cargo metadata for workspace `{}`: {:?}", path.display(), e))?;

	Ok(metadata.workspace_root.join("Cargo.toml").into())
}

/// Returns all package names of the given `workspace`.
fn workspace_packages(workspace: &Path) -> Result<impl Iterator<Item = cargo_metadata::Package>, String> {
	let metadata = cargo_metadata::MetadataCommand::new()
		.current_dir(workspace)
		.exec()
		.map_err(|e| format!("Failed to get cargo metadata for workspace `{}`: {:?}", workspace.display(), e))?;

	Ok(metadata.workspace_members.clone().into_iter().map(move |p| metadata[&p].clone()))
}
...
fn add_patches_for_packages(
	...
	let content =
		fs::read_to_string(cargo_toml).map_err(|e| format!("Could not read `{}`: {:?}", cargo_toml.display(), e))?;
	let mut doc = Document::from_str(&content)
		.map_err(|e| format!("Failed to parse `{}` as `Cargo.toml`: {:?}", cargo_toml.display(), e))?;
	...
	packages.try_for_each(|mut p| {
		...
		let path: PathBuf = if p.manifest_path.ends_with("Cargo.toml") {
			p.manifest_path.pop();
			p.manifest_path.into()
		} else {
			p.manifest_path.into()
		};
